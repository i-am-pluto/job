...
3. Primary path: run `/Users/parikshit/Documents/code/job/scripts/naukri_noperi_apply.py`.
...
7. Save company-site redirects to `data/pipeline.md`; use `generic-apply-agent` only if the CEO explicitly assigns budget.
8. Use the browser Naukri flow only as fallback when API login, token, or apply APIs fail.
...
11. Update `data/memory/naukri.md` with durable API behavior, keyword performance, blockers, and next-run improvements.
