...
- 2026-05-29 (morning): API adapter fully working with credentials in env. Full scan: 43 jobs, 4 direct applies (TCS, Persistent, Sandhata, Nustar walk-in events). External redirect rate ~65% (expected). CTC questionnaire blocks ~12 jobs. Unknown profile fields blocks ~25 jobs (DOB, INFY ID, microservices years, etc.). Base.pdf reused 4x, no tuning needed.
- 2026-05-29 (morning): NopeRi adapter stable; no login failures, token generation working. Duplicate detection vs. DB working (Sandhata correctly skipped by db_batch_insert) but agent still ATTEMPTED it — wasted 1 slot. Fix: re-read dup list right before building submission batch.
- 2026-05-29 (morning): Walk-in roles (TCS, Nustar) scored 4 and were applied. Lower strategic value than product/startup backend roles but valid fillers when quota not met.
- 2026-05-29 (morning): 122 jobs saved to pipeline for manual company-site follow-up. Direct-apply volume expected to remain low. Consider adding expected CTC to profile.md if user wants auto-answer support for CTC questionnaire blocks.
- 2026-05-29 (nightly 19:20-19:22): Naukri nightly-mode run APPLIED 48 JOBS (exceeded 15-job quota). Scanned 140+ jobs in single pass. Adapter did not respect --limit 15 flag; continued processing all matching jobs and submitted them all via db_batch_insert. Critical issue: adapter must stop at 15 applications in nightly mode. See issue tracking in code. Applications submitted: base.pdf (Java/backend), ai-backend.pdf (AI/Python), backend-systems.pdf (not used). Resume reuse rate: 100% (no tuning). Questionnaire and external-redirect blocks remain primary constraint. Recommends: (1) Fix adapter to honor --limit flag strictly in nightly mode; (2) re-read db.py list before building final submission batch to catch duplicates from other agents in same run; (3) add expected-ctc to profile for questionnaire fills.
