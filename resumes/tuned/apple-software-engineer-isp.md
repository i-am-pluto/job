...
Software Engineer with 2+ years building **backend systems**, **cloud-native infrastructure**, and **low-level systems software**. Hands-on with **C**, **Linux internals** (namespaces, cgroups, kernel interfaces), containerized runtime environments, and distributed compute on **Kubernetes**. Experienced in designing and owning production systems end-to-end, with strong foundations in systems programming, resource management, and cross-functional platform work.
...
- Implemented low-level kernel interfaces for process isolation, demonstrating direct experience with Linux systems internals and resource control subsystems.

**Indoor Navigation System** | Python, TensorFlow, OpenCV, YOLOv5, AWS IoT
- Built a **real-time indoor navigation system** with **camera-based scene understanding** using **OpenCV**, **YOLOv5**, and transfer learning (**ResNet50/VGG16**) for object detection and spatial reasoning.
- Deployed a Python inference pipeline with **AWS IoT** for sensor data streaming, optimized for low-latency real-time feedback.
...
Languages: C, Python, Java, Kotlin, JavaScript, SQL
Systems & Infrastructure: Linux (namespaces, cgroups), Docker, Kubernetes, Containers, Apache Spark, Airflow, Hadoop, Apache Kafka
Backend & Distributed Systems: Spring Boot, Microservices, REST APIs, System Design
Cloud & DevOps: AWS (Lambda, S3, IAM, Bedrock), Terraform, Snowflake, Databricks, GitLab CI, Jenkins, GitHub Actions
Observability & Databases: Prometheus, Grafana, Datadog, PostgreSQL, MySQL, MongoDB, Redis
